...
}},onComplete:function(){if(this.open){this.wrapper.removeClass("gf-menu-device-wrapper");}}}).hide());try{RokMediaQueries.on("(max-width: 959px)",this.mediaQuery.bind(this));
RokMediaQueries.on("(min-width: 960px)",this.mediaQuery.bind(this));}catch(b){if(typeof console!="undefined"){console.error('Error [Responsive Menu] while trying to add a RokMediaQuery "match" event',b);
...
if(!e&&!c){return;}if(d=="(min-width: 960px)"){e.inject(b.wrapper,"after");this.slide.wrapper.setStyle("display","none");this.toggler.setStyle("display","none");
