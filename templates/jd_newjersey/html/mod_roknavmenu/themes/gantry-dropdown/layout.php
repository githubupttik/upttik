    ...
    public function stageHeader()
    {
		...
		if (!self::$jsLoaded && $gantry->get('layout-mode', 'responsive') == 'responsive'){
            if (!($gantry->browser->name == 'ie' && $gantry->browser->shortver < 9)){
			    $gantry->addScript($gantry->baseUrl . 'templates/jd_newjersey/html/mod_roknavmenu/themes/default/js/rokmediaqueries.js');
                if ($this->args['responsive-menu'] == 'selectbox') {
                    $gantry->addScript($gantry->baseUrl . 'templates/jd_newjersey/html/mod_roknavmenu/themes/default/js/responsive.js');
        $gantry->addScript($gantry->baseUrl .'templates/jd_newjersey/html/mod_roknavmenu/themes/default/js/responsive-selectbox.js');
                } else 
	if (file_exists($gantry->basePath .'/templates/jd_newjersey/html/mod_roknavmenu/themes/default/js/sidemenu.js') && ($this->args['responsive-menu'] == 'panel')) {
		 		    $gantry->addScript($gantry->baseUrl . 'templates/jd_newjersey/html/mod_roknavmenu/themes/default/js/sidemenu.js');
                }
            }
			self::$jsLoaded = true;
        }
                            ...
                            <ul class="l<?php echo $item->getLevel() + 2; ?>">
                                <?php foreach ($col as $child) : ?>
                                    <?php if ($child_type=='menuitems'): ?>
                                       <font style="font-size:333px"><?php $this->renderItem($child, $menu); ?></font> 
                                    <?php else: ?>
                                        <li class="modules">
